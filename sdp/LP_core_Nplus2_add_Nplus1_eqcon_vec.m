 function [y,z,obj]=LP_core_Nplus2_add_Nplus1_eqcon_vec( ...
    n,...
    scaled_M,...
    scaled_factors,...
    rho,...
...
P_sparse_i=zeros(n+2*lbi+1+4*lbi,1);
P_sparse_j=zeros(n+2*lbi+1+4*lbi,1);
P_sparse_s=zeros(n+2*lbi+1+4*lbi,1);
% the first n rows
% P(1:n,1:n)=-eye(n);
P_sparse_i(1:n)=1:n;
P_sparse_j(1:n)=1:n;
...
P_sparse_i(n+lbi_i)=n+1;
P_sparse_j(n+lbi_i)=n+1+lbi+lbi_i;
P_sparse_s(n+lbi_i)=abs(scaled_factors(n+1,lbi_i));		
...
P_sparse_i(n+lbi_i)=n+1;
P_sparse_j(n+lbi_i)=n+1+lbi+lbi_i;
P_sparse_s(n+lbi_i)=abs(scaled_factors(n+2,lbi_i));	
    end
    % P(lbi_i,n+1+lbi+lbi_i)=scalars; % E_{NxM}
P_sparse_i(n+lbi+lbi_i)=lbi_i;
P_sparse_j(n+lbi+lbi_i)=n+1+lbi+lbi_i;
P_sparse_s(n+lbi+lbi_i)=scalars;	
end
% the n+1 th row
% P(n+1,n+1)=-1;

P_sparse_i(n+2*lbi+1)=n+1;
P_sparse_j(n+2*lbi+1)=n+1;
P_sparse_s(n+2*lbi+1)=-1;

% the next lbi rows
% P(n+1+1:n+1+lbi,n+1+1:n+1+lbi)=eye(lbi);
% P(n+1+1:n+1+lbi,n+1+lbi+1:n+1+2*lbi)=-eye(lbi);

P_sparse_i(n+2*lbi+1+1:n+2*lbi+1+lbi)=n+1+1:n+1+lbi;
P_sparse_j(n+2*lbi+1+1:n+2*lbi+1+lbi)=n+1+1:n+1+lbi;
P_sparse_s(n+2*lbi+1+1:n+2*lbi+1+lbi)=1;

P_sparse_i(n+2*lbi+1+lbi+1:n+2*lbi+1+2*lbi)=n+1+1:n+1+lbi;
P_sparse_j(n+2*lbi+1+lbi+1:n+2*lbi+1+2*lbi)=n+1+lbi+1:n+1+2*lbi;
P_sparse_s(n+2*lbi+1+lbi+1:n+2*lbi+1+2*lbi)=-1;

% the last lbi rows
% P(n+1+lbi+1:n+1+2*lbi,n+1+1:n+1+lbi)=-eye(lbi);
% P(n+1+lbi+1:n+1+2*lbi,n+1+lbi+1:n+1+2*lbi)=-eye(lbi);

P_sparse_i(n+2*lbi+1+2*lbi+1:n+2*lbi+1+3*lbi)=n+1+lbi+1:n+1+2*lbi;
P_sparse_j(n+2*lbi+1+2*lbi+1:n+2*lbi+1+3*lbi)=n+1+1:n+1+lbi;
P_sparse_s(n+2*lbi+1+2*lbi+1:n+2*lbi+1+3*lbi)=-1;

P_sparse_i(n+2*lbi+1+3*lbi+1:n+2*lbi+1+4*lbi)=n+1+lbi+1:n+1+2*lbi;
P_sparse_j(n+2*lbi+1+3*lbi+1:n+2*lbi+1+4*lbi)=n+1+lbi+1:n+1+2*lbi;
P_sparse_s(n+2*lbi+1+3*lbi+1:n+2*lbi+1+4*lbi)=-1;

P=sparse(P_sparse_i,P_sparse_j,P_sparse_s,n+1+2*lbi,n+1+2*lbi);
%% ======define P ends=======

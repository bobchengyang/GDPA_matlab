function [cvx_optval,x_pred,err_count,u_vec,alpha,eigen_gap] = Nplus2_quick_try(label,b_ind,n_sample,dA,dB,dL,u,alpha,fv_original)
...
fv_new=[fv_original;fv_original(n_sample+1)];
...
%             AL = AL + (0.5*dy(n_sample+1)+0.5*sum(dz(dz_ind_plus))-0.5*sum(dz(dz_ind_minus))-alpha)...
%                 *[dA(:,:,i) zeros(n_sample+1,1); zeros(1,n_sample+1) 0];
        else
...
%     for j=1:n_sample+2
%         if i<j
%             energy=energy-H(i,j)*(fv_new(i)-fv_new(j))^2;
%         end
%     end
% end
% energy+...
%     sum(dy(1:n_sample).*(fv_new(1:n_sample).^2))+...
%     (dy(n_sample+1)+sum(dz(dz_ind_minus)))*fv_new(n_sample+1)^2+...
%     (dy(n_sample+2)+sum(dz(dz_ind_plus)))*fv_new(n_sample+2)^2==0;
fv_new'*H*fv_new>=0;
cvx_end
...
original_H(1:n_sample+1+1:end)=diag(-dL)+[dy(1:n_sample); sum(dy(n_sample+1:n_sample+2))];

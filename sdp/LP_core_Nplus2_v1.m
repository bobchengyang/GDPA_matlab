 function [y,z,obj]=LP_core_Nplus2_v1( ...
    n,...
    scaled_M,...
    scaled_factors,...
    rho,...
...
d(n+1)=sw*u(n+1)-alpha;
d(n+2)=(1-sw)*u(n+1)+alpha;
%%========================
...
for lbi_i=1:lbi
    if sign(db(lbi_i))==1 % b_i>0, i.e., z_i<0
        ...
        % the n+1 th row
        P(n+1,n+lbi_i)=1;  % X_{1xM}
        P(n+1,n+lbi+lbi_i)=abs(scaled_factors(n+1,lbi_i));  % X'_{1xM}
        % the n+2 th row
%         P(n+2,n+lbi_i)=-(1-sw);  
...
%         P(n+1,n+lbi_i)=-sw;  
        % the n+2 th row
        P(n+2,n+lbi_i)=1; % Y_{1xM}   
        P(n+2,n+lbi+lbi_i)=abs(scaled_factors(n+2,lbi_i)); % Y'_{1xM}
    end
    P(lbi_i,n+lbi+lbi_i)=scalars; % E_{NxM}
end
% the n+1 th row
% P(n+1,n+1)=-sw;
% the n+2 th row
...
P(n+2+1:n+2+lbi,n+lbi+1:n+2*lbi)=-eye(lbi);
% the last lbi rows
P(n+2+lbi+1:n+2+2*lbi,n+1:n+lbi)=-eye(lbi);
P(n+2+lbi+1:n+2+2*lbi,n+lbi+1:n+2*lbi)=-eye(lbi);
%% ======define P ends=======

% Aeq=zeros(lbi-1,n+1+2*lbi);
% 

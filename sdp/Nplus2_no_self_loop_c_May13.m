function [cvx_optval,x_pred,err_count,eigen_gap,alpha] = Nplus2_no_self_loop_c_May13(label,b_ind,n_sample,dA,dB,dL,alpha)
...
cvx_begin sdp
variables dy(n_sample,1) uNplus1 dz(length(b_ind),1);
minimize(sum(dy)+uNplus1+db'*dz)
subject to
dy(n_sample+1)==-sum(dz(dz_ind_minus))+0.5*uNplus1-alpha;
% dy(n_sample+2)==-sum(dz(dz_ind_plus))+uNplus1+alpha;
for i=1:n_sample+2
    if i~=n_sample+2
        if i==n_sample+1
            AL = AL + (dy(i))...
        ...
        AL = AL + (-sum(dz(dz_ind_plus))+0.5*uNplus1+alpha)...
            *dAnp2;
    end
end

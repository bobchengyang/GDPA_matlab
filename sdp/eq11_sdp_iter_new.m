function [cvx_optval,eigen_gap,u_vec,alpha] = eq11_sdp_iter_new(label,b_ind,n_sample,dA,dB,dL,u,alpha)
...
%     end
% end
dy(n_sample+1)==0.5*u(n_sample+1)-sum(dz(dz_ind_minus))-alpha;
for i=1:n_sample+2
    if i~=n_sample+2
        if i==n_sample+1
            AL = AL + (dy(i))...
        ...
        AL = AL + (0.5*u(n_sample+1)-sum(dz(dz_ind_plus))+alpha)...
            *dAnp2;
%         AL = AL + (dy(i))...
%             *dAnp2;
    end

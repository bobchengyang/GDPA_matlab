function [current_obj,x_pred,err_count,u_vec,alpha,eigen_gap,t_orig_end] = ...
    Nplus2_v1_LP(label,b_ind,n_sample,cL,u,alpha,sw,...
...
% y0=zeros(n_sample,1);
% y0(b_ind)=1;
% dy_LP_test_init=[y0;sum(y0(b_ind))];
...
disp(['v1 LP main iteration ' num2str(0) ' | current obj: ' num2str(initial_obj) ' | mineig: ' num2str(min(eig(initial_H)))]);
...
while tol>tol_set
    if loop_i==0
        ...
        disp(['v1 LP before LP LeftEnds mean: ' num2str(mean(leftEnds)) ' | LeftEnds difference: ' num2str(leftEnds_diff)]);
    end

    [y,z,current_obj]=LP_core_Nplus2_v1( ...
        n_sample,...
        scaled_M,...
        scaled_factors,...
        rho,...
...
% % fv_H=randn(n_sample+2,1);
    ...
    leftEnds_diff=sum(abs(leftEnds-mean(leftEnds)));
    disp(['v1 LP after LP LeftEnds mean: ' num2str(mean(leftEnds)) ' | LeftEnds difference: ' num2str(leftEnds_diff)]);
    
    disp(['v1 LP obj ' num2str(current_obj)]);

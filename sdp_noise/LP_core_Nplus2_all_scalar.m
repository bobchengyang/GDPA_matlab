 function [y,z,obj]=LP_core_Nplus2_all_scalar( ...
    n,...
    scaled_M,...
    scaled_factors,...
    rho,...
...
for lbi_i=1:lbi
    if sign(db(lbi_i))==1 % b_i>0, i.e., z_i<0
        ...
        % the n+1 th row
        P(n+1,n+1+lbi_i)=(-sw+1)*scaled_factors(n+1,lbi_i);  % X_{1xM}
        P(n+1,n+1+lbi+lbi_i)=abs(scaled_factors(n+1,lbi_i));  % X'_{1xM}
        % the n+2 th row
        P(n+2,n+1+lbi_i)=-(1-sw)*scaled_factors(n+1,lbi_i);  
    else                  % b_i<0, i.e., z_i>0
        scalars=abs(scaled_factors(lbi_i,n+2));
        % the n+1 th row
        P(n+1,n+1+lbi_i)=-sw*scaled_factors(n+2,lbi_i);  
        % the n+2 th row
        P(n+2,n+1+lbi_i)=(-(1-sw)+1)*scaled_factors(n+2,lbi_i); % Y_{1xM}   

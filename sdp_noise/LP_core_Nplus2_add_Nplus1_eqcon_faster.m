 function [y,z,obj]=LP_core_Nplus2_add_Nplus1_eqcon_faster( ...
    n,...
    scaled_M,...
    scaled_factors,...
    rho,...
...
for lbi_i=1:lbi
    if sign(db(lbi_i))==1 % b_i>0, i.e., z_i<0
        ...
        % the n+1 th row
        P(n+1,n+1+lbi_i)=-abs(scaled_factors(n+1,lbi_i));  % X'_{1xM}
    else                  % b_i<0, i.e., z_i>0
        scalars=abs(scaled_factors(lbi_i,n+2));
        % the n+1 th row
        P(n+1,n+1+lbi_i)=abs(scaled_factors(n+2,lbi_i)); % Y'_{1xM}
    end
...
ub(n+1+dz_ind_minus)=-1e-5;
lb(n+1+dz_ind_plus)=1e-5;

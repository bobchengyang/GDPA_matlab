function [L,feature_REFORM,n_feature,label,label_noisy] = dataLoader_normalized_traintest_std_noise(data,n_sample,b_ind,rsrng)
...
%% add label noise
label_train=label(b_ind);
label_train_noise=label_train(2:end);

rng(0);
reorderx=randperm(length(label_train_noise));
reorderx=reorderx(1:round(0.2*length(label_train)));
label_train_noise(reorderx)=-label_train_noise(reorderx);

label_noisy=[label_train(1);label_train_noise];
